...
#генерируем токен
    def generate_token(self, expires_delta: Optional[timedelta] = None):
        ...
        return encode_jwt
#декодем и чекаем как раз на корректность
    def decode_token(self):
        try:
            decode_token = jwt.decode(
                self.token, SECRET_KEY, algorithms=[ALGORITM])
...
#отвечаем на ошибки get запросов(403 ,  и так далее)
    ...
    def verify_jwt(jwt_token: str):
        return True if jwt.decode(jwt_token, SECRET_KEY, algorithms=[ALGORITM]) is not None else False
